...
## 10.5 التحليل الديناميكي (Dynamic) — `[مُتحقَّق محليًا على محاكي]`

شُغِّل `Zero_LoaderV4.4.01.apk` فعليًا على محاكي Android (API 30، google_apis، تسريع KVM) ومُراقبة عبر `logcat` و`screencap`. النتائج:

1. **التشغيل والواجهة:** يُثبَّت ويعمل (PID حيّ، بلا انهيار). تظهر شاشة Zero Loader/Wolf ثم `LoginActivity` («Click Here for Get Key»).
2. **طلب أذونات وقت التشغيل:** يطلب فورًا الوصول إلى «photos, media, and files» (تخزين).
3. **مصادقة Firebase المجهولة — مؤكَّدة حيًّا:** سجّل `logcat` نجاح المصادقة المجهولة على مشروع المطوّر:
   `FirebaseAuth: Notifying auth state listeners about user ( IL4P7tfh8nT838gcKnZMSecHdZ22 )`
   — دليل تشغيلي مباشر على أن **المصادقة المجهولة مفعّلة وقابلة للاستغلال** (تطابق ثغرة §5).
4. **طلب صلاحية النافذة العائمة (Overlay):** يعرض «Please allow permision floating» — لازمة لرسم ESP/Aimbot فوق الشاشة (يطابق خدمات `Overlay/FloatAim/FloatLogo`).
5. **حدود الاختبار:** المكتبات الأصلية `arm64-v8a` فقط، ومحرّك BlackBox والحقن يحتاجان **روت + لعبة PUBG حقيقية**؛ لذا لم يتقدّم التدفّق إلى حقن الذاكرة/تنزيل `servernrot.zip` داخل المحاكي (بلا روت وبلا اللعبة). هذا متوقّع — التشغيل الكامل للغش يتطلّب جهازًا مرووتًا حقيقيًا، ولا يُنصح به على جهاز يحوي بيانات (راجع §7).

| طلب إذن التخزين وقت التشغيل | شاشة الدخول + طلب Overlay |
|---|---|
| ![perm](analysis_images/dyn_permission_request.png) | ![login](analysis_images/dyn_login_overlay.png) |

**الخلاصة الديناميكية:** التحليل الحيّ يؤكّد سلسلة البداية (تشغيل → أذونات → مصادقة Firebase مجهولة → طلب Overlay → إدخال المفتاح) ويثبت عمليًا ثغرة المصادقة المجهولة.
