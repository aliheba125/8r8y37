...
**⚠️ هذه قاعدة بيانات Firebase (`wolf-e99fb`)** — لتشغيل نظامك الخاص تحتاج Firebase خاص بك (القسم 6).

**ملاحظة أمنية:** api_key الخاص بـ Firebase ليس سراً بطبيعته (مصمم ليكون في العميل)، لكن **أمان قاعدة البيانات يعتمد كلياً على Security Rules**. إذا كانت القواعد مفتوحة، أي شخص يستطيع قراءة/كتابة الأكواد. لم تُختبر القاعدة الحية.
...
4. **sock daemon قديم (2018)** — قد لا يتوافق مع PUBG 3.7+ (احتمال بان). يحتاج rebuild من source.
